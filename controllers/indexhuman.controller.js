...
exports.createBodyIndexData2 = catchAsync(async (req, res) => {
  ...
  const newValue = {
    ...
    heart_rate,
    userId,
    TemperatureInF: +(1.8 * Temperature + 32).toFixed(2),
    health_status1: predict.rf_result,
    health_status2: predict.dt_result,
    health_status3: predict.nb_result,
    health_status4: predict.svm_result,
...
exports.getLastestForDoctor = catchAsync(async (req, res) => {
  const { patientId } = req.query;
  const healthIndex = await BodyIndexData.findOne({
    where: {
      userId: patientId,
    },
    order: [["createdAt", "DESC"]],
  });
  const disease = await BodyDiseases.findOne({
    where: {
      userId: patientId,
    },
    order: [["createdAt", "DESC"]],
